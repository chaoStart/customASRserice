...
# TEST 2: 生成测试用 WAV 文件
# ──────────────────────────────────────────────
info "TEST 2: 生成测试 WAV 文件（3秒静音，16kHz 单声道）..."
if ! command -v ffmpeg &>/dev/null; then
    fail "未找到 ffmpeg，无法生成测试 WAV 文件"
    exit 1
fi

ffmpeg -y -f lavfi -i "sine=frequency=440:duration=3" \
    -ar 16000 -ac 1 -sample_fmt s16 \
    "$TEST_WAV" > /dev/null 2>&1

if [ -f "$TEST_WAV" ]; then
    SIZE=$(du -h "$TEST_WAV" | cut -f1)
    pass "WAV 文件生成成功：$TEST_WAV（$SIZE）"
else
    fail "WAV 文件生成失败"
    exit 1
fi
...
# ──────────────────────────────────────────────
# TEST 3: 正常识别请求（合法 WAV 文件）
# ──────────────────────────────────────────────
info "TEST 3: 发送 WAV 文件进行识别..."
